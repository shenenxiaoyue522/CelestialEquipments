...
public enum BowType {
    ...
    public static final TooltipEntry longBow = TooltipEntry.define("Bow type: long bow");

    @SubscribeTooltip(id = "bow_type_short")
    public static final TooltipEntry shortBow = TooltipEntry.define("Bow type: short bow");

    public MutableComponent getLang() {
        MutableComponent text;
        switch (this) {
            case LONG_BOW -> text = longBow.withColor(ChatFormatting.BLUE);
            case SHORT_BOW -> text = shortBow.withColor(ChatFormatting.BLUE);
            default -> throw new IncompatibleClassChangeError();
        }
        return text;
    }
